...
static void sec_otg_set_id_state(int id)
{
	...
	if (motg->phy.state != OTG_STATE_UNDEFINED) {
		...
		* but, 'system_nrt_wq' is deprecated. It changed to schedule_work() fucntion.
		*/
...
{
	pr_info("%s: ULPI: 0x%x 0x%x 0x%x 0x%x - original\n", __func__,
	...
	pr_info("%s: ULPI: 0x%x 0x%x 0x%x 0x%x - modified\n", __func__,
			ulpi_read(&otg->phy, 0x80),
			ulpi_read(&otg->phy, 0x81),
			ulpi_read(&otg->phy, 0x82),
			ulpi_read(&otg->phy, 0x83));
