...
struct dm_verity {
	...
	unsigned int shash_descsize;/* the size of temporary space for crypto */
...
static void dirty_hash(struct bio *bio, int error)
{
	...
	for (i = 0; i < (io->iter.bi_size >> v->data_dev_block_bits); i++) {
		...
		if (sector_to_block(v, io->iter.bi_sector) + (sector_t)i == 0) {
...
static inline unsigned int bytes_to_block(struct dm_verity *v, unsigned int bytes)
{
...
static int dirty_map(struct dm_target *ti, struct bio *bio)
{
	...
	if (bio->bi_iter.bi_size & ((1 << v->data_dev_block_bits) - 1) ||
		...
		DMERR("Not page size IO: size:%u, sector:%llu", bio->bi_iter.bi_size,
			(u64)bio->bi_iter.bi_sector);
		return -EIO;
	}

	if (sector_to_block(v, bio->bi_iter.bi_sector) + bytes_to_block(v, bio->bi_iter.bi_size) > v->data_blocks) {
		DMERR("Out of range. sector:%llu, size:%u",
			(u64)bio->bi_iter.bi_sector, bio->bi_iter.bi_size);
		return -EIO;
	}
...
static void dirty_one_level_up(struct dm_verity *v, sector_t block, int source_level,
		sector_t *hash_block, unsigned int *offset)
{
	...
	*offset = (block_offset & ((1 << v->hash_per_block_bits)-1))
...
static int finish_verity_tree(struct dm_verity *v)
{
	...
	for (i = 1; i < v->levels; i++) {
		...
		for (block_num = v->hash_level_block[i - 1]; nr_blocks > 0; nr_blocks--, block_num++) {
			...
			if (dirty_update_hash(v, block_num, desc, result)) {
				DMERR("Error calculating hash for block %llu", (u64)block_num);
				goto free_desc;
			}
			hash_block_buf = dm_bufio_read(v->bufio, hash_block, &buf);
			if (unlikely(IS_ERR(hash_block_buf))) {
				DMERR("Error getting hash block %llu in dtr", (u64)hash_block);
				buf = NULL;
				goto free_desc;
			}
	...
	if (dirty_update_hash(v, hash_block, desc, result)) {
		DMERR("Failed to calculate root hash. Block: %llu", (u64)hash_block);
		goto free_desc;
	}
...
static int dirty_ioctl(struct dm_target *ti, unsigned int cmd,
			unsigned long arg)
{
	struct dm_verity *v =  ti->private;
	struct {
...
static int dirty_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	struct dm_verity *v;
	unsigned int num;
	...
	if (strcmp(argv[9], "-")) {
		v->salt_size = (unsigned int)strlen(argv[9]) / 2;
	...
	v->verify_wq = alloc_workqueue("kdirtyd", WQ_CPU_INTENSIVE | WQ_MEM_RECLAIM | WQ_UNBOUND,
					1 /* num_online_cpus() */);
	if (!v->verify_wq) {
