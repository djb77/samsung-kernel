...
#ifdef CONFIG_SENSORS_FINGERPRINT_DUALIZATION
int FP_CHECK; /* extern variable init */
#endif
...
#ifndef ENABLE_SENSORS_FPRINT_SECURE
static DEVICE_ATTR(retain_pin, 0664, vfsspi_retain_show, vfsspi_retain_store);
#endif
...
static void vfsspi_work_func_debug(struct work_struct *work)
{
	...
	pr_info("%s ldo:%d, sleep:%d, irq:%d, tz:%d, type:%s, cnt_irq:%d, adm: %d\n",
		__func__,
		ldo_value, gpio_get_value(g_data->sleep_pin),
		gpio_get_value(g_data->drdy_pin),
		g_data->tz_mode,
		sensor_status[g_data->sensortype + 2],
		g_data->cnt_irq, g_data->detect_mode);
}
...
#ifdef CONFIG_SENSORS_FINGERPRINT_DUALIZATION
	FP_CHECK = 0; /* extern variable init */
#endif
