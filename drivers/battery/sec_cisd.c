...
bool sec_bat_cisd_check(struct sec_battery_info *battery)
{
	...
	if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
		...
		if (is_cisd_check_type(battery->cable_type) && incur_val.intval > pcisd->current_max_thres &&
			chgcur_val.intval > pcisd->charging_current_thres && battery->current_now > battery->pdata->full_chg_current_margin &&
		...
		if (is_cisd_check_type(battery->cable_type) && battery->siop_level == 100 &&
			battery->current_now > battery->pdata->full_chg_current_margin && battery->charging_mode == SEC_BATTERY_CHARGING_2ND) {
		...
		if (is_cisd_check_type(battery->cable_type) && battery->siop_level == 100 &&
			battery->current_now >= battery->pdata->full_chg_current_margin && ((battery->status == POWER_SUPPLY_STATUS_FULL &&
	...
	pr_info("cisd - stt:%d, cp:%d/%d/%d, cpmm:%d/%d/%d, dcpt:%d, ovc:%d, rct:%d\n",
		pcisd->state, pcisd->curr_cap_max, pcisd->err_cap_high_thr, pcisd->err_cap_low_thr, pcisd->data[CISD_DATA_CAP_MIN],
...
void sec_battery_cisd_init(struct sec_battery_info *battery)
{
	...
	battery->cisd.data[CISD_DATA_CAP_MAX] = -1;
	battery->cisd.data[CISD_DATA_CAP_MIN] = -1;
	battery->cisd.data[CISD_DATA_RECHARGING_TIME] = 0x7FFFFFFF;
	battery->cisd.data[CISD_DATA_CAP_ONCE] = -1;
	battery->cisd.data[CISD_DATA_CAP_PER_TIME] = -1;
	battery->cisd.data[CISD_DATA_ERRCAP_LOW] = -1;
	battery->cisd.data[CISD_DATA_ERRCAP_HIGH] = -1;
