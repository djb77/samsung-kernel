                          ...
                          volatile cmn_msg_hdr_t *msg, int msglen)
{
	...
	for (step = 1; step <= PCIE_D2H_SYNC_NUM_OF_STEPS; step++) {
		for (tries = 1; tries <= PCIE_D2H_SYNC_WAIT_TRIES; tries++) {
			/* For checking incorrect item_len */
			if (prot->d2hring_rx_cpln.item_len > D2HRING_RXCMPLT_ITEMSIZE) {
					DHD_ERROR(("%s : Invalid Data in xorcsum \n", __FUNCTION__));
					DHD_ERROR(("%s : step (%d), tries (%d), prot (%p), msglen(%d), num_words(%d) \n",
					 __FUNCTION__, step, tries, prot, msglen, num_words));
