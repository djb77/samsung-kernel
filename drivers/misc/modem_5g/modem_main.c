...
static int (*parse_dt_pdata)(struct device *,
				struct modem_data *);
					...
					struct modem_data *pdata)
{
	...
	mif_dt_read_string(np, "mif,regulator_ldo1", pdata->regulator_ldo1);
	mif_dt_read_string(np, "mif,regulator_ldo2", pdata->regulator_ldo2);
	mif_dt_read_string(np, "mif,regulator_ldo3", pdata->regulator_ldo3);
	mif_dt_read_string(np, "mif,regulator_ldo4", pdata->regulator_ldo4);
	mif_dt_read_string(np, "mif,regulator_ldo5", pdata->regulator_ldo5);
	mif_dt_read_string(np, "mif,regulator_ldo11", pdata->regulator_ldo11);
	mif_dt_read_string(np, "mif,regulator_ldo12", pdata->regulator_ldo12);
	mif_dt_read_string(np, "mif,regulator_ldo13", pdata->regulator_ldo13);
	mif_dt_read_string(np, "mif,regulator_ldo14", pdata->regulator_ldo14);
	mif_dt_read_string(np, "mif,regulator_ldo15", pdata->regulator_ldo15);
	mif_dt_read_string(np, "mif,regulator_ldo16", pdata->regulator_ldo16);
	mif_dt_read_string(np, "mif,regulator_ldo17", pdata->regulator_ldo17);
	mif_dt_read_string(np, "mif,regulator_sw2", pdata->regulator_sw2);

	/* GPIO_BUCK_EN1 */
	pdata->gpio_buck_en1 = of_get_named_gpio(np,
						"mif,gpio_buck_en1", 0);
	if (!gpio_is_valid(pdata->gpio_buck_en1)) {
		mif_err("buck_en1: Invalied gpio pins\n");
		return -EINVAL;
	}

	mif_err("gpio_buck_en1: %d\n", pdata->gpio_buck_en1);
	ret = gpio_request(pdata->gpio_buck_en1, "BUCK_EN1");
	if (ret)
		mif_err("fail to request gpio %s:%d\n", "BUCK_EN1", ret);
...
#if 0
static int parse_dt_dummy_rmnet_pdata(struct device *dev, struct device_node *np,
				 struct modem_data *pdata)
{
	struct device_node *child = NULL;
	size_t size = sizeof(struct modem_io_t) * pdata->dummy_num_rmnets;
	int i = 0;

	mif_err("<KTG> dummy_num_rmnets:%d\n", pdata->dummy_num_rmnets);

	pdata->dummy_rmnets = devm_kzalloc(dev, size, GFP_KERNEL);
	if (!pdata->dummy_rmnets) {
		mif_err("dummy_rmnets: failed to alloc memory\n");
		return -ENOMEM;
	}
...
static int parse_shmem_dt_pdata(struct device *dev,
				 struct modem_data *pdata)
{
				 ...
				 struct modem_data *pdata)
{
	...
	if (parse_dt_regulator_pdata(dev, dev->of_node, pdata)) {
		mif_err("DT error: failed to get regulator\n");
		panic("eijfw");
		goto error;
	}

#if 0
	iodevs = of_get_child_by_name(dev->of_node, "dummy_rmnets");
	if (!iodevs) {
		mif_err("DT error: failed to get child node(%s)\n",
			"dummy_rmnet");
		goto error;
	}

	if (parse_dt_dummy_rmnet_pdata(dev, iodevs, pdata)) {
		mif_err("DT error: failed to parse iodevs\n");
		goto error;
	}
#endif
...
static struct modem_data *modem_if_parse_dt_pdata(struct device *dev)
{
	...
	if (parse_dt_pdata) {
		if (parse_dt_pdata(dev, pdata)) {
			mif_err("DT error: failed to parse pdata\n");
			goto error;
		}
	}
