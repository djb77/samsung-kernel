...
#define DEBUG_KTG
//#define DEBUG_TX_TEST

#define MODEM_PCIE_CH_NUM	1

#define MODEM_PCIE_VENDOR_ID	0x144d
#define MODEM_PCIE_DEVICE_ID	0xa63b

#define MODEM_PCI_BAR_NAME	"mif_pcie"

#define PCI_OB_DB_PENDING_0	0x88
#define PCI_OB_DB_PENDING_1	0x89
...
#define MODEM_PCI_BAR_USAGE ((1 << MP_BAR_0) | (1 << MP_BAR_2) | (1 << MP_BAR_4))

#define pcie_readb(base, offset)	readb_relaxed((volatile void *)	\
					(uintptr_t)(base+offset))
#define pcie_writeb(base, offset, val) do {							\
	u8 cmp_val;										\
	u32 idx = 5;										\
	do {											\
		writeb_relaxed(val,								\
		...
		wmb();										\
		cmp_val = pcie_readb(base, offset);						\
		if (cmp_val != val) {								\
			mif_err("PCIe read did not return same value as was written\n");	\
		} else {									\
			break;									\
		}										\
	} while (--idx);									\
...
#define pcie_readl(base, offset)	 readl_relaxed((volatile void *)			\
						(uintptr_t)(base+offset))
#define pcie_writel(base, offset, val) do {							\
	u32 cmp_val;										\
	u32 idx = 5;										\
	do {											\
		writel_relaxed(val,								\
				(volatile void *)(uintptr_t)(base + offset)); 			\
		wmb();										\
		cmp_val = pcie_readl(base, offset);						\
		if (cmp_val != val) {								\
			mif_info("PCIe read did not return same value as was written\n");	\
		} else {									\
			break;									\
		}										\
	} while (--idx);									\
...
#define pcie_memset(base, size) do {								\
	u32 offset = 0;										\
	u32 cnt = size / 4;									\
	do {											\
		pcie_writel(base, offset, 0);							\
		offset += 4;									\
	} while (--cnt);									\
												\
	cnt = size % 4;										\
	while (cnt) {										\
		pcie_writeb(base, offset, 0);							\
		offset += 1;									\
		cnt -= 1;									\
	}											\
} while(0)
...
#define pci_map_single(cookie, address, size, dir)      virt_to_phys(address)	//virt_to_bus(address)
#define pci_unmap_single(cookie, address, size, dir)

/*
struct modem_pcie_dma_trans {
	modem_pcie_dma_ring_desc ring_desc_tx;
};
*/
struct modem_pcie_dev_info {

	void __iomem *bar_base[MAX_BAR_NUM];
...
struct mem_link_device {
	...
	struct pci_dev *pdev;
	struct pci_driver pdev_drv;

	struct modem_pcie_dev_info modem_pcie_dev;

	struct work_struct polling_timer;
