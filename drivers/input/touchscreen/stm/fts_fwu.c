...
int FTS_Check_DMA_Done(struct fts_ts_info *info)
{
	...
	do {
		info->fts_read_reg(info, &regAdd[0],
				2, (unsigned char *)val, 1);
...
static int fts_fw_burn_d3(struct fts_ts_info *info, unsigned char *fw_data)
{
	...
	int rc;
	const unsigned long int FTS_TOTAL_SIZE = FW_IMAGE_SIZE_D3;
	...
	while (i < size) {
		...
		while ((j < DRAM_LEN) && (j < dataLen)) {
			...
			memcpy(&buf[cnt],
					&fw_data[sizeof(struct fts_header) + i],
					WRITE_CHUNK_SIZE_D3);
		...
		input_info(true, info->dev,
				"%s: Write to Flash - Total %ld bytes\n",
				__func__, i);
		...
		buf[2] = 0x00;
		buf[3] = 0x00;
		buf[4] = (CODE_ADDR_START + ((k * DRAM_LEN) >> 2)) & 0xFF;
		// k * 64 * 1024 / 4
		buf[5] = (CODE_ADDR_START + ((k * DRAM_LEN) >> (2+8))) & 0xFF;
		// k * 64 * 1024 / 4 / 256
...
int fts_fw_wait_for_event(struct fts_ts_info *info, unsigned char eid)
{
	...
	rc = -1;
	while (info->fts_read_reg(info, &regAdd, 1,
				(unsigned char *)data, FTS_EVENT_SIZE)) {
...
int fts_fw_wait_for_event_D3(struct fts_ts_info *info,
		unsigned char eid0, unsigned char eid1)
{
	int rc;
	unsigned char regAdd;
	unsigned char data[FTS_EVENT_SIZE];
	...
	regAdd = READ_ONE_EVENT;
	rc = -1;
	while (info->fts_read_reg(info, &regAdd,
...
void fts_execute_autotune(struct fts_ts_info *info)
{
...

	{
		...
		msleep(300);
		fts_fw_wait_for_event_D3(info,
				STATUS_EVENT_MUTUAL_AUTOTUNE_DONE, 0x00);
		info->fts_command(info, SELF_AUTO_TUNE);
		msleep(300);
		fts_fw_wait_for_event_D3(info,
				STATUS_EVENT_SELF_AUTOTUNE_DONE_D3, 0x00);

#ifdef FTS_SUPPORT_WATER_MODE
		fts_fw_wait_for_event(info,
				STATUS_EVENT_WATER_SELF_AUTOTUNE_DONE);
...

	{
		...
		if (ret < 0)
			input_info(true, info->dev,
					"%s: Flash Back up PureAutotune Fail(Set)\n",
					__func__);

		msleep(20);
		fts_fw_wait_for_event(info,
				STATUS_EVENT_PURE_AUTOTUNE_FLAG_WRITE_FINISH);
	}
...
#ifdef PAT_CONTROL
	if ((info->cal_count == 0) || (info->cal_count == 0xFF)
			|| (magic_cal == true)) {
		input_info(true, info->dev,
		...
		if (magic_cal) {
			...
			//get_pat_data(info);
			info->cal_count++;
			if (info->cal_count > PAT_MAX_MAGIC)
				info->cal_count = PAT_MAX_MAGIC;
		...
		input_info(true, info->dev,
				"%s: tune_fix_ver [0x%4X]\n", __func__,
				info->tune_fix_ver);
	} else {
		input_info(true, info->dev,
				"%s: DO NOT CALIBRATION(%d)\n", __func__,
				info->cal_count);
	}
#endif
...
const int fts_fw_updater(struct fts_ts_info *info,
		...
		int restore_cal)
{
	...
	fw_main_version =
		(header->released_ver[0] << 8) + (header->released_ver[1]);
			...
			if (fw_main_version == info->fw_main_version_of_ic) {
				...
				if (restore_cal) {
					if (info->cal_count >= PAT_MAGIC_NUMBER
						&& info->board->pat_function
						== PAT_CONTROL_PAT_MAGIC)
						magic_cal = true;
				}
				input_info(true, info->dev,
						"%s: cal_count(%d) pat_function dt(%d) restore_cal(%d) magic_cal(%d)\n",
						__func__, info->cal_count,
						info->board->pat_function,
						restore_cal,
						magic_cal);
...
int fts_fw_update_on_probe(struct fts_ts_info *info)
{
	...
	if (info->board->firmware_name) {
		if (gpio_is_valid(info->board->device_id)
				&& gpio_get_value(info->board->device_id)) {
			retval = fts_read_analog_chip_id(info, ANALOG_ID_FTS8);
			if (retval == 1)
				info->firmware_name = FW_IMAGE_NAME_FTS8;
			else
				info->firmware_name =
					info->board->firmware_name;
		} else
			info->firmware_name = info->board->firmware_name;
	} else {
		input_info(true, info->dev,
				"%s:firmware name does not declair in dts\n",
				__func__);
		goto exit_fwload;
	}
	...
	retval = request_firmware_direct(&fw_entry, fw_path, info->dev);
	if (retval) {
	...
	info->config_version_of_bin =
		(fw_data[CONFIG_OFFSET_BIN_D3] << 8) +
		fw_data[CONFIG_OFFSET_BIN_D3 - 1];
	...
	//change cal_count from 0 to magic number
	//to make virtual pure auto tune
	if ((info->cal_count == 0
				&& info->board->pat_function
				== PAT_CONTROL_PAT_MAGIC)
			|| info->board->pat_function
...
static int fts_load_fw_from_ums(struct fts_ts_info *info)
{
	...
	fp = filp_open(FTS_DEFAULT_UMS_FW, O_RDONLY, 00400);
	if (IS_ERR(fp)) {
...
static int fts_load_fw_from_ffu(struct fts_ts_info *info)
{
	...
	if (fw_entry->size
			!= (FW_IMAGE_SIZE_D3 + sizeof(struct fts_header)
				+ SIGNEDKEY_SIZE)) {
		input_err(true, info->dev,
