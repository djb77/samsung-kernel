...
void score_dpmu_print_dbg_intr(void)
{
	...
	score_note("DPMU:%32s: %8x\n",
			"DBG_INTR_ENABLE",
			readl(dpmu_sfr + DBG_INTR_ENABLE));
	score_note("DPMU:%32s: %8x %8x\n",
...
{
	score_note("DPMU:%32s: %8x %8x %8x %8x %8x\n",
...
{
	score_note("DPMU:%32s: %8x %8x %8x %8x\n",
...
void score_dpmu_print_stall_count(void)
{
	...
	score_note("DPMU:%32s: %8x %8x %8x %8x %8s\n",
...
void score_dpmu_print_debug(void)
{
	...
	for (idx = 0; idx < 32; idx += 4)
		score_note("DPMU:%26s %02d-%02d: %8x %8x %8x %8x\n",
...
void score_dpmu_stall_enable_on(int on)
{
	...
	stall_en = on;
#else
	score_note("stall count can not be used on EVT0\n");
#endif
	score_leave();
}
