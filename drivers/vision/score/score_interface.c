...
static void __score_interface_scq_read(struct score_interface *itf)
{
	...
	frame = score_frame_get_process_by_id(framemgr, temp.frame_id);
	if (!frame) {
		spin_unlock_irqrestore(&framemgr->slock, flags);
		return;
	}
	...
	spin_lock_irqsave(&framemgr->slock, flags);
	if (score_frame_get_pending_count(framemgr)) {
		pending_frame = score_frame_get_first_pending(framemgr);
		if (pending_frame) {
...
static void __score_interface_check_swi(struct score_interface *itf)
{
	...
	if (swi_status & DBG_ERR_INTR_STATUS) {
		for (idx = 0; idx < 10; ++idx)
			score_err("PC: MC(%#8x) KC1(%#8x) KC2(%#8x)\n",
					readl(itf->sfr + MC_PROG_COUNTER),
					readl(itf->sfr + KC1_PROG_COUNTER),
					readl(itf->sfr + KC2_PROG_COUNTER));
	}
		...
		int interrupt)
{
	...
	if (interrupt & INTERRUPT_BIT(INT_SWI)) {
		...
		if (swi_status & DBG_ERR_INTR_STATUS) {
			...
			framemgr = &itf->framemgr;
			spin_lock_irqsave(&framemgr->slock, flags);
			score_frame_flush_process(framemgr, -EINTR);
			wake_up_all(&framemgr->done_wq);
			score_interface_target_halt(itf, SCORE_KNIGHT1);
			score_interface_target_halt(itf, SCORE_KNIGHT2);
...
int score_interface_probe(struct score_system *system)
{
	...
	if (IS_ERR_OR_NULL(itf->isr_task)) {
