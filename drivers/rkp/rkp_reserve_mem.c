...
{
	rkp_mem[0].start_add = RKP_PHYS_MAP_START;
	rkp_mem[1].start_add = RKP_VMM_START;
	rkp_mem[2].start_add = RKP_LOG_START;
	//RKP Secure Log. It is not used. Just alloc memory.
	rkp_mem[3].start_add = RKP_SEC_LOG_START;
	rkp_mem[4].start_add = RKP_DASHBOARD_START;
	rkp_mem[5].start_add = RKP_ROBUF_START;

	rkp_mem[0].size = RKP_PHYS_MAP_SIZE;
	rkp_mem[1].size = RKP_VMM_SIZE;
	rkp_mem[2].size = RKP_LOG_SIZE;
	rkp_mem[3].size = RKP_SEC_LOG_SIZE;
	rkp_mem[4].size = RKP_DASHBOARD_SIZE;
	rkp_mem[5].size = RKP_ROBUF_SIZE;
}
...
#endif
			pr_err("%s: RKP failed reserving size 0x%x at base 0x%x\n",
				__func__, (unsigned int)rkp_mem[i].size, (unsigned int)rkp_mem[i].start_add);
			return -1;
		}
		pr_info("RKP :%s, base:0x%x, size:0x%x\n",
