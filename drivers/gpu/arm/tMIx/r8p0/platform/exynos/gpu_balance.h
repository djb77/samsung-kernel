...
#define MALI_GPU_CONTROL_WAIT(reg, val, timeout) do {if (mali_wait_reg(kbdev, GPU_CONTROL_REG(reg), val, timeout) != true) {return false; } } while (0)

#define MALI_JOB_IRQ_WAIT(val, timeout) do {if (mali_wait_reg(kbdev, JOB_IRQ_RAWSTAT, val, timeout) != true) {return false; } } while (0)
