...
void kbase_jm_wait_for_zero_jobs(struct kbase_context *kctx)
{
	...
	if (!wait_event_timeout(kctx->jctx.sched_info.ctx.is_scheduled_wait,
				!kbase_ctx_flag(kctx, KCTX_SCHEDULED), (unsigned int) msecs_to_jiffies(3000)))
	{ /* 3sec timeout -> BUG() */
		dev_err(kbdev->dev, "%s: kctx scheduled out fail during 3sec\n", __func__);
		BUG();
	}
#endif
...
int kbase_job_slot_init(struct kbase_device *kbdev)
{
	...
	kbdev->hwaccess.backend.reset_workq =
		...
		alloc_workqueue("Mali reset workqueue", RESET_WQ_PARAM | 0, 1);
...
static void kbasep_try_reset_gpu_early_locked(struct kbase_device *kbdev)
{
	...
	if (atomic_cmpxchg(&kbdev->hwaccess.backend.reset_gpu,
		...
		dev_err(kbdev->dev, "%s: unexpected reset race\n", __func__);
