...
struct decon_device {
	...
	bool dsr_on;
#endif
	struct v4l2_subdev *panel_sd[2];
	struct panel_state *panel_state[2];
...
{
	if ((decon->panel_state[0] != NULL) &&
		(decon->panel_state[0]->hmd_on))
		return false;
	else
		return true;
}
...
#define S3CFB_DUAL_DISPLAY_BLANK _IOW('F', 300, struct dual_blank_data)
