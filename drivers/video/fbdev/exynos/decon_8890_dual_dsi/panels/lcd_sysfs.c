...
static struct device_attribute backlight_dev_attrs[] = {
	__ATTR(weakness_hbm_comp, 0664, weakness_hbm_show, weakness_hbm_store),
	__ATTR(auto_brightness, 0644, auto_brightness_show, auto_brightness_store),
};
...
void lcd_init_sysfs(struct dsim_device *dsim)
{
	...
	/*sysfs located /sys/class/backlight/... */
	for (i = 0; i < ARRAY_SIZE(backlight_dev_attrs); i++) {
		if (device_create_file(&dsim->priv.bd->dev, backlight_dev_attrs + i)) {
			dev_err(&dsim->lcd->dev, "ERROR : Failed to create device file %d:%s\n",
				i, backlight_dev_attrs[i].attr.name);
		}
	}
	return;
}
