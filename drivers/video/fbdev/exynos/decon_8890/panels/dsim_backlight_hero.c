...
static int dsim_panel_set_tset(struct dsim_device *dsim)
{
	int tset = 0;
	...
	dsim_info("%s temperature: %d, tset: %d\n",
			__func__, dsim->priv.temperature, tset);
...
{
	u8 *elvss = NULL;
	unsigned char SEQ_ELVSS[ELVSS_LEN + 1] = {0, };
	struct panel_private *panel = &dsim->priv;

	elvss = get_elvss_from_index(dsim, panel->br_index, panel->caps_enable);
	if (elvss == NULL) {
		dsim_err("%s : failed to get elvss value\n", __func__);
		return;
	}
	SEQ_ELVSS[0] = ELVSS_REG;
	memcpy(&SEQ_ELVSS[1], panel->elvss_set, ELVSS_LEN);
	memcpy(&SEQ_ELVSS[2], elvss, ELVSS_CMD_CNT);
	SEQ_ELVSS[1] = dsim_panel_set_tset(dsim);


	SEQ_ELVSS[ELVSS_LEN] += dsim_panel_get_elvssoffset(dsim);
	if (dsim_write_hl_data(dsim, SEQ_ELVSS, ELVSS_LEN + 1) < 0)
...
static int dsim_panel_set_vint(struct dsim_device *dsim, int force)
{
	...
	struct panel_private* panel = &(dsim->priv);
	unsigned char SEQ_VINT[VINT_LEN] = {VINT_REG, 0xAB, 0x1E};
