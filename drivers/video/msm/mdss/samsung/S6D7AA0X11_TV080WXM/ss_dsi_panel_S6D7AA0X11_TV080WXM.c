...
static struct dsi_panel_cmds * mdss_brightness_tft_pwm(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	if (unlikely(vdd->debug_data->pwm_tuning))
		vdd->candela_level = vdd->bl_level;

	LCD_INFO("Scaled Level : %d\n", vdd->candela_level);
...
static void mdnie_init(struct samsung_display_driver_data *vdd)
{
	...
	if (mdnie_data) {
		...
		mdnie_data[0].COLOR_BLIND_MDNIE_2 = COLOR_BLIND_2;
		mdnie_data[0].RGB_SENSOR_MDNIE_1 = NULL;
		mdnie_data[0].RGB_SENSOR_MDNIE_2 = NULL;
		mdnie_data[0].NIGHT_MODE_MDNIE_1 = NIGHT_MODE_2;
		...
		mdnie_data[0].COLOR_BLIND_MDNIE_SCR = COLOR_BLIND_2;
		...
		mdnie_data[0].EMAIL_AUTO_MDNIE = NULL;
