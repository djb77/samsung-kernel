...
#endif
#define writelx(r, v) writel_relaxed((v), (r))
...
void klog(void)
{
	...
	if (debug_mdp->reg_log.last*4 >= debug_mdp->reg_log.len)
		return;
	if (debug_mdp->reg_log.len == 0)
		return;
...
int  mdss_debug_init(void)
{
	...
	dump_size = log_buff_len + event_desc_len + reg_log_len + clock_state_len + sizeof(struct debug_mdp);
#ifdef __KERNEL__
	...
	if (!__debug_mdp_phys || (__debug_mdp_phys == (u32)debug_mdp)) {
		...
		strlcpy(debug_mdp->marker, "*#$$_START_OF_MDP_DEBUG_DUMP##$", sizeof("*#$$_START_OF_MDP_DEBUG_DUMP##$"));
		if (debug_mdp != NULL)
