...
#include "fimc-is-companion_address_c3_v2.h"
#else
...
#define FIMC_IS_COMPANION_LSC_ADDR			(MEM_GRAS_B)
#define FIMC_IS_COMPANION_LSC_SIZE			5880
#define FIMC_IS_COMPANION_LSC_PARAMETER_ADDR		(grasTuning_uParabolicCenterX_front)
#define FIMC_IS_COMPANION_LSC_PARAMETER_SIZE		228
#define FIMC_IS_COMPANION_LSC_ADDR_FRONT		(MEM_GRAS_B_front)
#define FIMC_IS_COMPANION_LSC_SIZE_FRONT		5880
#define FIMC_IS_COMPANION_LSC_PARAMETER_ADDR_FRONT	(grasTuning_uParabolicCenterX)
...
request_fw:
	...
	if (!strcmp(name, COMP_FW_EVT0)) {
		...
		info("%s version : %s\n", name, companion_rev);
		fimc_is_ischain_version(FIMC_IS_BIN_COMPANION, companion_rev, sizeof(companion_rev)-1);
...
#ifdef CONFIG_COMPANION_FACTORY_VALIDATION
void fimc_is_comp_i2c_config(struct i2c_client *client, bool onoff)
{
...
int fimc_is_comp_run_CRC_on_Close(void *core_data)
{
	struct fimc_is_core *core = core_data;
	int ret = 0;
	...
	ret = fimc_is_comp_i2c_write(core->client0, 0x6808, 0x0001);
	if (ret) {
		err("fimc_is_comp_run_CRC_on_Close i2c fail");
	}
...
bool fimc_is_comp_check_CRC_Valid(void *core_data) {
	struct fimc_is_core *core = core_data;
	bool ret = true;
#ifdef CONFIG_COMPANION_AUTOMATIC_CRC_ON_CLOSE
	char *pcalc_crc;
	char calc_crc[FIMC_IS_COMPANION_CRC_SIZE * FIMC_IS_COMPANION_CRC_OBJECT];
	u16 data[2];
	int count = 0;
	...
	usleep_range(10000, 10000); /* delay for CRC run */

	while (count < 10) {
		usleep_range(2000, 2000); /* delay for CRC run */
		fimc_is_comp_single_write(core, 0x642C, 0x0004);
		fimc_is_comp_single_write(core, 0x642E, 0xCBB0);
		fimc_is_comp_single_read(core, 0x6F12, &data[0]);
		fimc_is_comp_single_read(core, 0x6F12, &data[1]);
		info("[check]crc value : %x%x\n", data[0], data[1]);
		memcpy(pcalc_crc, &data[0], 2);
		isValid[0] = (FIMC_IS_COMPANION_VALID_CRC & 0x00ff);
		isValid[1] = (FIMC_IS_COMPANION_VALID_CRC & 0xff00) >> 8;
		if (memcmp((const void *)pcalc_crc, (const void *)isValid, 2)) {
	...
	if ((ret < 0) && (retry_count > 0)) {
		...
		if (ret < 0) {
			if (GET_SENSOR_STATE(specific->standby_state, SENSOR_STATE_COMPANION) == SENSOR_STATE_OFF) {
				switch (core->vender.companion_crc_error) {  /* error recovery on boot */
					case COMPANION_CRC_ERROR_INIT_ON_BOOT:
						core->vender.companion_crc_error = COMPANION_CRC_ERROR_OCCUR_ON_BOOT;
						break;
					case COMPANION_CRC_ERROR_RESET_ON_BOOT:
						core->vender.companion_crc_error = COMPANION_CRC_ERROR_FATAL_ON_BOOT;
						break;
				}
			}
		} else {
			core->vender.companion_crc_error = COMPANION_CRC_ERROR_NONE_ON_BOOT;
		}
	}
