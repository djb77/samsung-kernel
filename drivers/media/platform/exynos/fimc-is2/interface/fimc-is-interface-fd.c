	...
	for (i = 0; i < subip_num; i++) {
		...
		for (setfile_index = 0; setfile_index < lib_data->setfile.num; setfile_index++) {
			if (i != fd_offset) {
				...
				dbg_lib(3, "set_file index: %d\n", setfile_index);
				dbg_lib(3, "setfile_version(%#x), flag(%#x), min_face(%d), max_face(%d)\n",
						fd_setfile->preview.setfile_version, fd_setfile->preview.flag,
						fd_setfile->preview.min_face, fd_setfile->preview.max_face);
				dbg_lib(3, "central_lock_w(%d), central_lock_h(%d), central_max_face(%d)\n",
				...
				dbg_lib(3, "frame_per_lock(%d), smooth(%d), max_face_cnt(%d),"
						KERN_CONT "keep_faces_over_frame(%d)\n",
				...
				dbg_lib(3, "boost_fd_vs_fd(%d), boost_fd_vs_speed(%d), frame_per_lock_no_face(%d)\n",
						fd_setfile->preview.boost_fd_vs_fd,
						fd_setfile->preview.boost_fd_vs_speed,
						fd_setfile->preview.frame_per_lock_no_face);
				dbg_lib(3, "keep_limit_no_face(%d), min_face_size_feature_detect(%d),"
						KERN_CONT "max_face_size_feature_detect(%d)\n",
...
int fimc_is_lib_fd_thread_init(struct fimc_is_lib *fd_lib)
{
	...
	kthread_init_worker(&fd_lib->worker);
	...
	for (cnt = 0; cnt < MAX_FIMC_IS_LIB_TASK; cnt++) {
		...
		kthread_init_work(&fd_lib->work[cnt].work, fimc_is_lib_fd_fn);
	}
...
void fimc_is_lib_fd_trigger(struct fimc_is_lib *fd_lib)
{
	...
	kthread_queue_work(&fd_lib->worker, &fd_lib->work[work_index].work);
}
