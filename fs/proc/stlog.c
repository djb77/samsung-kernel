...
static ssize_t stlog_write(struct file *file, const char __user *buf,
					size_t count, loff_t *ppos)
{
	return do_stlog_write(STLOG_ACTION_WRITE, buf,
					count, STLOG_FROM_READER);
}


loff_t stlog_llseek(struct file *file, loff_t offset, int whence)
{
	return (loff_t)do_stlog(STLOG_ACTION_SIZE_BUFFER,
					0, 0, STLOG_FROM_READER);
}
...
static const char DEF_STLOG_VER_STR[] = "1.0.4\n";
...
{
	proc_create("stlog", 0444, NULL, &stlog_operations);
	proc_create("stlog_pipe", 0444, NULL, &stlog_pipe_operations);
	proc_create("stlog_version", 0444, NULL, &stlog_ver_operations);
	return 0;
}
...
module_param_named(stlog_cpu, stlog_cpu_id, bool, 0644);
...
#endif
module_param_named(stlog_pid, stlog_pid, bool, 0644);
...
static unsigned int ringbuf_cpu = UINT_MAX;
...
static void ringbuf_store(enum ringbuf_flags flags, const char *text,
			u16 text_len, u8 cpu_id, struct task_struct *owner)
{
	struct ringbuf *msg;
	u32 size, pad_len;

#ifdef DEBUG_STLOG
	pr_debug("[STLOG] %s stlog_seq %llu stlog_idx %lu ringbuf_first_seq %llu ringbuf_first_idx %lu ringbuf_next_seq %llu ringbuf_next_idx %lu\n",
			__func__, stlog_seq, stlog_idx, ringbuf_first_seq,
			ringbuf_first_idx, ringbuf_next_seq, ringbuf_next_idx);
#endif
	...
	while (ringbuf_first_seq < ringbuf_next_seq) {
		...
		if (ringbuf_next_idx > ringbuf_first_idx)
			free = max(ringbuf_buf_len - ringbuf_next_idx,
							ringbuf_first_idx);
		else
	...
	if (ringbuf_next_idx + size + sizeof(struct ringbuf) >=
						ringbuf_buf_len) {
		memset(ringbuf_buf + ringbuf_next_idx, 0,
...
#endif
module_param_named(stlog_time, stlog_time, bool, 0644);
...
static size_t stlog_print_prefix(const struct ringbuf *msg,
					bool ringbuf, char *buf)
{
	size_t len = 0;
...
static size_t stlog_print_text(const struct ringbuf *msg,
		enum ringbuf_flags prev, bool ringbuf, char *buf, size_t size)
{
	...
	do {
		...
		if (buf) {
			if (stlog_print_prefix(msg, ringbuf, NULL) +
						text_len + 1 >= size - len)
				break;

			if (prefix)
				len += stlog_print_prefix(msg, ringbuf,
								buf + len);
...
static int stlog_print(char __user *buf, int size)
{
	...
	while (size > 0) {
		...
		n = stlog_print_text(msg, ringbuf_prev, false, text,
...
#ifdef DEBUG_STLOG
		pr_debug("[STLOG] %s OPEN stlog_seq %llu stlog_idx %lu ringbuf_first_seq %llu ringbuf_first_idx %lu ringbuf_next_seq %llu ringbuf_next_idx %lu\n",
			__func__, stlog_seq, stlog_idx, ringbuf_first_seq,
			ringbuf_first_idx, ringbuf_next_seq, ringbuf_next_idx);
#endif
		break;
	case STLOG_ACTION_READ:	/* cat /proc/stlog_pipe */
#ifdef DEBUG_STLOG
		pr_debug("[STLOG] %s READ stlog_seq %llu stlog_idx %lu ringbuf_first_seq %llu ringbuf_first_idx %lu ringbuf_next_seq %llu ringbuf_next_idx %lu\n",
			__func__, stlog_seq, stlog_idx, ringbuf_first_seq,
			ringbuf_first_idx, ringbuf_next_seq, ringbuf_next_idx);
#endif
		error = -EINVAL;
...
#ifdef DEBUG_STLOG
		pr_debug("[STLOG] %s READ_ALL stlog_seq %llu stlog_idx %lu ringbuf_first_seq %llu ringbuf_first_idx %lu ringbuf_next_seq %llu ringbuf_next_idx %lu\n",
			__func__, stlog_seq, stlog_idx, ringbuf_first_seq,
			ringbuf_first_idx, ringbuf_next_seq, ringbuf_next_idx);
#endif
		error = -EINVAL;
...
#ifdef DEBUG_STLOG
		pr_debug("[STLOG] %s SIZE_BUFFER %lu\n", __func__,
						ringbuf_buf_len);
#endif
